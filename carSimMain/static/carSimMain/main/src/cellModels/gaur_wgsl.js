...
export const gaurCoreCompute = /*wgsl*/`
    ...
    var INaL__hl_inf:f32 = 1.00000/(1.00000+exp((states.vm+91.0000)/6.10000));

    var ICaL__d_inf:f32 = 1.00000/(1.00000+exp( - (states.vm - constants.ICaL__vhalf_d)/6.20000));
    var ICaL__tau_d:f32 = 0.600000+1.00000/(exp(  - 0.0500000*(states.vm+6.00000))+exp( 0.0900000*(states.vm+14.0000)));

    var IKr__tau_xr:f32 = 12.9800+1.00000/( 0.365200*exp((states.vm - 31.6600)/3.86900)+ 4.12300e-05*exp( - (states.vm - 47.7800)/20.3800));
    var IKr__xr_inf:f32 = 1.00000/(1.00000+exp( - (states.vm+56.8000)/17.8000));

    var ITo__alpha_aa:f32 = 0.0250000/(1.00000+exp((states.vm+58.0000)/5.00000));
    var ITo__beta_aa:f32 = 1.00000/( 5.00000*(1.00000+exp((states.vm+19.0000)/ - 9.00000)));
    var ITo__aa_inf:f32 = ITo__alpha_aa / (ITo__alpha_aa + ITo__beta_aa);
    var ITo__tau_aa:f32 = 1.00000 / (ITo__alpha_aa + ITo__beta_aa);

    var ICaL__f_inf:f32 = 1.00000/(1.00000+exp((states.vm - constants.ICaL__vhalff)/4.90000))+0.350000/(1.00000+exp((45.0000 - states.vm)/20.0000));
    var ICaL__ff_inf:f32 = ICaL__f_inf;
    var ICaL__tau_ff:f32 = 7.00000+1.00000/( 0.00450000*exp( - (states.vm+20.0000)/10.0000)+ 0.00450000*exp((states.vm+20.0000)/10.0000));

    var ICaL__fs_inf:f32 = ICaL__f_inf;
    var ICaL__tau_fs:f32 = 70.0000+1.00000/( 3.50000e-05*exp( - (states.vm+5.00000)/4.00000)+ 3.50000e-05*exp((states.vm+5.00000)/6.00000));

    var IKs__tau_xs1:f32 = 300.000+1.00000/( 1.00000e-06*exp((states.vm+50.0000)/20.0000)+ 0.0400000*exp( - (states.vm+50.0000)/20.0000));
    var IKs__xs1_inf:f32 = 1.00000/(1.00000+exp( - (states.vm - 25.1000)/37.1000));

    var IKs__tau_xs2:f32 = 1.00000/( 0.0100000*exp((states.vm - 50.0000)/20.0000)+ 0.0193000*exp( - (states.vm+66.5400)/31.0000));
    var IKs__xs2_inf:f32 = IKs__xs1_inf;

    var INaL__aml:f32;
    if (states.vm == -47.1300) {
        INaL__aml =  0.32 * 10.0;
    } else {
        INaL__aml =  0.32 * -((states.vm+47.13)/(exp(-0.1*(states.vm+47.13)) - 1.00000));
    }
    var INaL__bml:f32 =  0.0800000*exp( - states.vm/11.0000);
    var INaL__ml_inf:f32 = INaL__aml/(INaL__aml+INaL__bml);
    var INaL__tau_ml:f32 = 1.00000/(INaL__aml+INaL__bml);

    var I_Na__m_inf:f32 = 1.00000/( (1.00000+exp(( - 56.8600 - states.vm)/9.03000))*(1.00000+exp(( - 56.8600 - states.vm)/9.03000)));
    var I_Na__aa_m:f32 = 1.00000/(1.00000+exp(( - 60.0000 - states.vm)/5.00000));
    var I_Na__bb_m:f32 = 0.100000/(1.00000+exp((states.vm+35.0000)/5.00000))+0.100000/(1.00000+exp((states.vm - 50.0000)/200.000));
    ...
    var I_Na__h_inf:f32 = 1.00000/( (1.00000+exp((states.vm+71.5500)/7.43000))*(1.00000+exp((states.vm+71.5500)/7.43000)));
    var I_Na__aa_h:f32;
    var I_Na__bb_h:f32;
    var I_Na__aa_j:f32;
    var I_Na__bb_j:f32;
    if (states.vm >= -40.0) {
        I_Na__aa_h =  0.0;
        I_Na__bb_h =  0.770000/( 0.130000*(1.00000+exp(-(states.vm+10.6600)/11.1000)));
        I_Na__aa_j =  0.0;
        I_Na__bb_j = ( 0.600000*exp( 0.0570000*states.vm))/(1.00000+exp(  - 0.100000*(states.vm+32.0000)));
    } else {
        I_Na__aa_h =  0.057*exp(-(states.vm+80.0)/6.8);
        I_Na__bb_h =  2.70000*exp( 0.0790000*states.vm)+ 310000.*exp( 0.348500*states.vm);
        I_Na__aa_j = ( (  - 25428.0*exp( 0.244400*states.vm) -  6.94800e-06*exp(  - 0.0439100*states.vm))*(states.vm+37.7800))/(1.00000+exp( 0.311000*(states.vm+79.2300)));
        I_Na__bb_j = ( 0.0242400*exp(  - 0.0105200*states.vm))/(1.00000+exp(  - 0.137800*(states.vm+40.1400)));
    }
    ...
    var CaMK__vfrt:f32 = ( states.vm*constants.cell__F)/( constants.cell__R*constants.cell__T);
    var ICaL__PhiCaL:f32;
    if (states.vm==0.0) {
        ICaL__PhiCaL =  ( ( 2.00000*constants.cell__F)*( states.ionic_concentrations__cass*exp( 2.00000*(( states.vm*constants.cell__F)/( constants.cell__R*constants.cell__T))) -  0.341000*cell__cao))* 1.0 ;
    } else {
        ICaL__PhiCaL =  ( ( 2.00000*constants.cell__F)*( states.ionic_concentrations__cass*exp( 2.00000*(( states.vm*constants.cell__F)/( constants.cell__R*constants.cell__T))) -  0.341000*cell__cao))*( 2.00000*CaMK__vfrt)/(exp( 2.00000*CaMK__vfrt) - 1.00000);
    }
    ...
    var IK1__rk1:f32 = 1.00000/(1.00000+exp(((states.vm+79.3000) -  2.60000*constants.cell__ko)/19.6000));
    var curr_IK1:f32 =  ( ( constants.IK1__GK1*pow((constants.cell__ko/5.40000), 1.0 / 2))*IK1__rk1)*(states.vm - CaMK__EK);
    var IKb__xkb:f32 = 1.00000/(1.00000+exp( - (states.vm - 14.4800)/18.3400));
    var curr_IKb:f32 =  ( constants.IKb__GKb*IKb__xkb)*(states.vm - CaMK__EK);
    var IKr__rkr:f32 = 1.00000/(1.00000+exp((states.vm+22.0000)/15.0000));
    var curr_IKr:f32 =  ( ( ( constants.IKr__GKr*pow((constants.cell__ko/5.40000), 1.0 / 2))*states.IKr__xr)*IKr__rkr)*(states.vm - CaMK__EK);
    ...
    var curr_IKs:f32 =  ( ( ( constants.IKs__GKs*IKs__KsCa)*states.IKs__xs1)*states.IKs__xs2)*(states.vm - CaMK__EKs);
    var INaK__Knai:f32 =  constants.INaK__Knai0*exp(( ( constants.INaK__delta*states.vm)*constants.cell__F)/( ( 3.00000*constants.cell__R)*constants.cell__T));
    ...
    var INaK__Knao:f32 =  constants.INaK__Knao0*exp(( ( (1.00000 - constants.INaK__delta)*states.vm)*constants.cell__F)/( ( 3.00000*constants.cell__R)*constants.cell__T));
    ...
    var INaCa_i__hna:f32 = exp(( ( constants.INaCa_i__qna*states.vm)*constants.cell__F)/( constants.cell__R*constants.cell__T));
    ...
    var INaCa_i__hca:f32 = exp(( ( constants.INaCa_i__qca*states.vm)*constants.cell__F)/( constants.cell__R*constants.cell__T));
    ...
    var curr_INaL:f32 =  ( ( ( ( constants.INaL__GNaL*states.INaL__ml)*states.INaL__ml)*states.INaL__ml)*states.INaL__hl)*(states.vm - CaMK__ENa);
    var curr_INab:f32;
    if (states.vm==0.0) {
    ...
    var curr_INa:f32 =  ( ( ( ( ( constants.I_Na__GNa*states.I_Na__m)*states.I_Na__m)*states.I_Na__m)*states.I_Na__h)*states.I_Na__j)*(states.vm - CaMK__ENa);
    ...
    var ITo__rito2:f32 = 1.00000/(1.00000+exp( - (states.vm+10.0000)/5.00000));
    var curr_ITo:f32 =  ( ( ( constants.ITo__Gto*states.ITo__aa)*ITo__rito2)*ITo__kito2)*(states.vm - CaMK__ECl);
