...
export function computeShaderMonodomainSurfDiffusion(cellModel, nNodes){
    ...
    return /*wgsl*/`
        ...
        fn comp_monodomain_diffusion(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
...
export function computeShaderMonodomainSurfReaction(cellModel, nNodes){
    ...
    return /*wgsl*/`
        ...
        @binding(0) @group(0) var<storage, read>       stim          : array<Stim, ${nNodes}>;
        ...
        @binding(4) @group(0) var<storage, read_write> results : array<f32>;


        @compute @workgroup_size(255)
        fn comp_monodomain_reaction(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
...
export function computeShaderMonodomainSurfUpdateVmFromDifussion(cellModel, nNodes){
    ...
    return /*wgsl*/`
        ...
        @compute @workgroup_size(255)
        fn comp_monodomain_update_V_from_diffusion(@builtin(global_invocation_id) GlobalInvocationID : vec3<u32>) {
            ...
            states[idx].vm = states[idx].vm + ((states[idx].divG_gradV * integ.dt/2 * 100000) / (constants.beta * constants.cm));
            results[idx] = states[idx].vm;
            
            vois[idx] = (states[idx].vm - visual_params.voi_min) / (visual_params.voi_max - visual_params.voi_min);  

            states[idx].t += integ.dt/2;
